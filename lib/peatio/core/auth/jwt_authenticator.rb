...
module Peatio::Core::Auth
  ...
  #
  #   auth = Peatio::Core::Auth::JWTAuthenticator.new(rsa_public)
  ...
  class JWTAuthenticator
    ...
    #
    # @raise [Peatio::Core::Auth::Error] If token is invalid or can't be verified.
    def authenticate!(token)
      ...
      case error
      when Peatio::Core::Auth::Error
        raise(error)
      else
        raise(Peatio::Core::Auth::Error, error.message)
      end
    end
    ...
    def decode_and_verify_token(token)
      ...
      raise(Peatio::Core::Auth::Error, "Failed to decode and verify JWT: #{e.message}")
    end
  end
end
