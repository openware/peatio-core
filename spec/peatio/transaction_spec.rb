...
describe Peatio::Core::Transaction do
  context :validations do
    ...
    context :presence do
      ...
      it 'initialize valid transaction' do
        expect(Peatio::Core::Transaction.new(transaction_attrs).valid?).to be_truthy
      end

      it 'validates presence' do
        required_attrs.each do |attr|
          expect(Peatio::Core::Transaction.new(transaction_attrs.except(attr)).valid?).to be_falsey
        end
      end
    ...
    context :numericality do
      ...
      it 'validates amount to be number' do
        ...
        expect(Peatio::Core::Transaction.new(transaction_attrs).valid?).to be_falsey
      end
  ...
  context :initialize do
    ...
    it 'sets default status to pending' do
      ...
      expect(Peatio::Core::Transaction.new(transaction_attrs).status).to eq 'pending'
    end

    it 'converts status to string' do
      transaction_attrs[:status] = :success
      expect(Peatio::Core::Transaction.new(transaction_attrs).status).to eq 'success'
    end
  end
  ...
  context :status do
    ...
    it 'wraps status to StringInquirer' do
      transaction = Peatio::Core::Transaction.new(transaction_attrs)
